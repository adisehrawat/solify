...
impl PdaDetector {
    pub fn detect_pdas(&self, registry: &AccountRegistry, program_id: Pubkey) -> Result<Vec<PdaInit>> {
        let mut pda_inits = Vec::new();

        for account in &registry.accounts {
            if account.is_pda {
                let pda_init = self.create_pda_init(account, program_id)?;
    ...
    fn create_pda_init(&self, account: &crate::analyzer::dependency_analyzer::AccountInfo, program_id: Pubkey) -> Result<PdaInit> {
    ...
    fn estimate_account_space(&self, account: &crate::analyzer::dependency_analyzer::AccountInfo) -> u64 {
    ...
    fn sort_pdas_by_dependencies(&self, pda_inits: &mut Vec<PdaInit>, registry: &AccountRegistry) -> Result<()> {
        ...
        for (i, pda_init) in pda_inits.iter().enumerate() {
            let account = registry.get_account(&pda_init.account_name)
                .ok_or(SolifyError::AccountConstraintParseFailed)?;
            ...
            match seed.seed_type {
                OutputSeedType::AccountKey => {
                    ...
                    if registry.get_account(&seed.value).is_none() {
                        msg!("Warning: PDA seed references unknown account: {}", seed.value);
                        return Ok(false);
                    }
                }
                OutputSeedType::Argument => {
                    ...
                    msg!("PDA uses argument seed: {}", seed.value);
                }
